...
- **Exact eigenvalue method** (`symbolic_marginal_entropies_exact_su9_pair`): 
  Works for 3×3 matrices. Eigenvalues come from cubic formula → nested radicals.
  Differentiating `log(nested_radical)` creates `d(radical)/radical` expressions
  that SymPy cannot simplify, leading to expression blowup.
- **Taylor approximation** (`symbolic_marginal_entropies_taylor_su9_pair`):
  Uses H(ρ) ≈ log(d) - (d/2)·Tr[(ρ - I/d)²] for ρ near I/d
  - For d=3: coefficient is **3/2** (not 1/2!)
  - Fast to differentiate (polynomial expressions)
  - Error O(||θ||⁴) - acceptable for small θ

**Key lesson**: Name functions clearly to distinguish approximate from exact!
...
### Timing Reference (4 symbolic parameters)
| Operation | Time |
|-----------|------|
| Density matrix | ~0.2s |
| Partial trace | ~0.2s |
| Eigenvalues 3×3 | ~0.04s |
| Differentiate polynomial | ~0.05s |
| Differentiate log(eigenvalue) | MINUTES (eigenvalues contain nested radicals from cubic) |
