...
**Note on d≥4**: Analytic treatment unlikely to be practical. The numerical code
(`QuantumExponentialFamily`) handles arbitrary d efficiently. Focus analytic 
effort on qubits and qutrits which cover the most important physical systems.

**Roadmap**:
1. ✅ Qutrits (d=3): Complete with block structure optimization
2. 🔲 Qubits (d=2): Next priority - simpler, good validation
3. ❌ General d: Likely impractical for analytic treatment
   - Block structures are d-specific
   - Eigenvalue formulas don't generalize (quadratic for d=2, needs block structure for d=3)
   - Structure constants scale as (d²-1)³
   - Numerical code handles general d; analytic for d=2,3 covers key physical cases
