...
2. Computes the full flow Jacobian M (already implemented in `MatrixExponentialFamily.jacobian()`)
3. Decomposes M = A + S
...
**In `qig.exponential_family.MatrixExponentialFamily`:**
...
3. **Symmetric/antisymmetric decomposition** (add to `MatrixExponentialFamily`):
...
1. **Computing the Jacobian M** (implemented in `MatrixExponentialFamily.jacobian()`):
   ```
   M = -G - (∇G)[θ] + ν∇²C + a(∇ν)ᵀ
   ```
...
3. **Extend `MatrixExponentialFamily`** (test-driven):
...
- **No changes** to existing API of `MatrixExponentialFamily`, `InaccessibleGameDynamics`
...
- [x] Add `symmetric_part()` to `MatrixExponentialFamily`
- [x] Add `antisymmetric_part()` to `MatrixExponentialFamily`
- [x] Add `verify_degeneracy_conditions()` to `MatrixExponentialFamily`
