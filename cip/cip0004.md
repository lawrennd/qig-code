...
### Phase 3: Quality Assurance and Documentation 🔄 **PARTIALLY COMPLETE**

- [x] Test suite validation completed
  - All 17 converted test files running with tolerance framework
  - 37/45 tests passing in converted BKM/cumulant tests (8 expected failures exposing real bugs)
...
- [ ] Improve clustering of tests within files that are obviously testing a particular decomposition of underlying code.
  - **See**: Backlog task `2025-11-27_consolidate-test-suite-structure.md` for detailed consolidation plan
  - Current: 19 files with unclear responsibility and overlap
  - Proposed: 10 files mirroring qig module structure (core, exponential_family, dynamics, etc.)
  - Benefits: Better discoverability, reduced redundancy, self-documenting architecture
- [ ] Migration guide for future test development
- [ ] Training materials for tolerance selection

## References

**Converted Test Files (17 files):**

All test files now use the tolerance framework with scientifically justified bounds:

1. `test_inaccessible_game.py` - Main comprehensive test suite (45 tests)
2. `test_pair_exponential_family.py` - Pair basis exponential family (23 tests)
3. `test_pair_numerical_validation.py` - Pair basis numerical validation (5 tests, with known failures)
4. `test_commuting_bkm.py` - Commuting/diagonal BKM validation (25 tests)
5. `test_non_commuting_bkm.py` - Non-commuting BKM diagnostics (3 tests, with known failures)
6. `test_nondiagonal_commuting_bkm.py` - Rotated commuting operators (9 tests)
7. `test_third_cumulant.py` - Third cumulant tensor tests (4 tests)
8. `test_third_cumulant_symmetry.py` - Cumulant symmetry validation (1 test)
9. `test_jacobian.py` - Full Jacobian implementation (5 tests)
10. `test_jacobian_analytic.py` - Analytic Jacobian computation (4 tests)
11. `test_lagrange_multiplier_gradient.py` - ∇ν implementation (4 tests)
12. `test_marginal_entropy_gradient.py` - Constraint gradient ∇C (5 tests)
13. `test_constraint_hessian.py` - Constraint Hessian ∇²C (4 tests)
14. `test_constraint_hessian_duhamel.py` - High-precision Duhamel Hessian (2 tests)
15. `test_theta_only_constraint.py` - θ-only optimization method (18 tests)
16. `test_quantum_qutrit.py` - Qutrit system validation (5 tests)
17. `test_inaccessible_game.py` - Core dynamics and integration tests
