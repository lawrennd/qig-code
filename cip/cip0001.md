...
- `inaccessible_game_quantum.py`
  ...
  - matrix exponential family (`MatrixExponentialFamily`) and BKM/Fisher information.
...
2. **Consolidate and simplify module structure**
   - Factor the code into a small number of clearly named modules, for example:
     ...
     - `qig/exponential_family.py` – `MatrixExponentialFamily` and BKM metric.
...
- [x] Migrate the main library classes (`MatrixExponentialFamily`, `InaccessibleGameDynamics`) into `qig.exponential_family` and `qig.dynamics`
...
- [x] **Created `qig/pair_operators.py`**: Clean separation of pair-based operators
  ...
  - `pair_basis_generators(d)`: su(d²) for entangled pairs
...
- [x] **Created `qig/duhamel.py`**: High-precision quantum derivatives
  - Duhamel integral for ∂ρ/∂θ (machine precision)
...
- [x] **Created `test_pair_exponential_family.py`** (16 tests):
  ...
  - Block-diagonal Fisher metric (cross-pair ~10⁻¹⁶)
  - Computational scaling verification

- [x] **Created `test_pair_numerical_validation.py`** (10 tests):
  - ∂ρ/∂θ validation: ~1-3×10⁻⁵ error
  - Fisher metric G: ~4×10⁻⁴ error
  - Constraint gradient ∇C: ~9×10⁻⁶ error
  - Constraint Hessian ∇²C: ~6×10⁻⁴ error
  - **Jacobian M: ~1.3×10⁻⁵ error**
  - Dynamics verification: F ≠ 0, Gθ ≠ -a for entangled systems
...
- [ ] **Audit and update legacy dynamics scripts**:
  ...
  - [ ] **Key question**: Were these scripts supposed to study entangled systems?
    - If YES → Need to update to use `pair_basis=True` and su(d²) operators
    - If NO → Document that they study separable states only
  ...
  - [ ] Update any hardcoded assumptions about ν = -1 or Gθ = -a
...
- ✅ 26 new tests (all passing) with tight tolerances
- ✅ Pair-based operators enable genuine entanglement (I > 0)
- ✅ Analytic Jacobian validated for both local and pair systems
- ✅ Fisher metric block-diagonal structure confirmed
- ✅ Verification code examples in documentation

**Status: CLOSED (2026-06-13)** — Core refactoring complete. The "critical remaining work" on legacy scripts was superseded by CIP-0002 (Audit and Migrate Legacy Scripts to Support Entangled Systems), which was implemented the same day. All root scripts (`inaccessible_game_quantum.py`, `quantum_qutrit_n3.py`, `validate_qutrit_optimality.py`) now import from `qig.*` and `quantum_qutrit_n3.py` has been fully migrated to wrap `qig.exponential_family` with pair operators. Further consolidation of root scripts is tracked separately in backlog task `2025-12-01_consolidate-root-python-scripts`.
...
- **Problem**: `MatrixExponentialFamily` with default initialization → separable states only
- **Action needed**:
  1. Check if any dynamics experiments assume entanglement exists
  2. If studying entangled systems → convert to `pair_basis=True`
  3. Update `InaccessibleGameDynamics` if it makes assumptions about ν = -1 or Gθ = -a
...
- **Questions to answer**:
  ...
  2. Does entropy-time reparametrization change when C ≠ H (entanglement present)?
...
- **Action needed**:
  ...
  2. If entangled → update to pair basis and re-run experiments
  3. Update any plots/analysis that assume I = 0

### `validate_qutrit_optimality.py`
- **Current state**: Qutrit optimality claim with local operators (`MatrixExponentialFamily(n_sites, d)`)
...
- **Action needed**:
  ...
  2. If yes → **must use pair basis** with su(9) generators (80 parameters)
...
- **Action needed**:
  ...
  3. If migrating:
     - Replace custom code with `MatrixExponentialFamily(n_pairs=N, d=3, pair_basis=True)`
...
1. **Check paper**: What do experiments claim to study?
   - Separable states only? → Local operators are correct
   - Entangled pairs? → **Must use pair operators**
2. **Check each script's initialization**:
   ```python
   # Search for patterns like:
   MatrixExponentialFamily(n_sites=..., d=...)  # ← Local operators
   # vs
   MatrixExponentialFamily(n_pairs=..., d=..., pair_basis=True)  # ← Pair operators
   ```
3. **Check for hardcoded assumptions**:
   - ν = -1 (only true for local operators)
   - Gθ = -a (structural identity, only holds for local operators)
...
1. **Update initialization** for entangled systems:
   ...
   exp_fam = MatrixExponentialFamily(n_sites=2, d=3)  # 2 qutrits, 16 params
   
   # New (pair operators, entanglement possible)
   exp_fam = MatrixExponentialFamily(n_pairs=1, d=3, pair_basis=True)  # 1 qutrit pair, 80 params
   ```

2. **Update dynamics code**:
   - Remove hardcoded ν = -1
   - Remove hardcoded Gθ = -a
   - Use `jacobian(theta, method='duhamel')` for corrected Jacobian

3. **Update analysis code**:
   - Check for I > 0 (entanglement present)
   - Use ∇C ≠ ∇H (different gradients for entangled systems)
...
- `qig/exponential_family.py` - `MatrixExponentialFamily` with BKM metric, Jacobian, constraints
...
- `qig/duhamel.py` - High-precision quantum derivatives
  - Duhamel integral for machine-precision ∂ρ/∂θ
...
**Total: 26+ comprehensive tests covering all quantum gradients with tight tolerances (~10⁻⁵)**
...
1. **Clear API separation**: Local operators vs pair operators
   ...
   exp_fam_pairs = MatrixExponentialFamily(n_pairs=1, d=2, pair_basis=True)
   ```

2. **High-precision quantum derivatives**:
   - ∂ρ/∂θ: ~1-3×10⁻⁵ error (Duhamel method)
   - Fisher G: ~4×10⁻⁴ error, block-diagonal ~10⁻¹⁶
   - Jacobian M: ~1.3×10⁻⁵ error

3. **Validated entanglement capability**:
   - Local operators: I = 0 always (C = H)
   - Pair operators: I > 0 (genuine entanglement)
   - Structural identity Gθ = -a breaks for entangled systems
...
- ✅ `2025-11-22_remove-numerical-gradients` - All gradients now analytic
- ✅ `2025-11-22_analytic-jacobian-implementation` - Full Jacobian implemented
- ✅ `2025-11-22_entangled-pair-exponential-family` - Pair basis implemented
- 🔄 `2025-11-22_qig-docs-and-tests-refactor` - Remaining: update validation docs
