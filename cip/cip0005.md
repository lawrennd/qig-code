...
- [x] Accepted → Working simulation with coarse graining
- [x] Implemented → Core simulation working, SEA needs improvement
...
- [x] *Gradual dephasing with cumulative time evolution* (γ_τ = γ_per_block * τ)
...
- [x] *Theory-compliant entropy growth*: 0.000 → 0.588 → 0.689 → 0.693 (saturation)

### In Progress 🔄
- [ ] *Implement proper SEA dynamics*: ∂τρ = i[log ρ*, ρ] with ρ* = argmax S(ρ̃) subject to fixed marginals
...
### Advanced Theoretical Extensions 🔬 (Jaynes/Natural-Parameter Framework)
- [ ] *0: Hard constraint projector* - Store P_hard that separates exact vs relaxing constraints
- [ ] *A-1: Natural parameters* - Compute canonical parameters θ_j = log p_j - ψ in soft coordinates
- [ ] *A-2: SEA verification* - Track θ̇_soft and verify θ̇ = -G_∥θ numerically
- [ ] *B: Fisher as effective masses* - Eigenvalues λ_k as curvature of entropy vs constraint
- [ ] *C: Automatic charge identification* - Classify hard (λ=0) and soft (λ<λ_cut) charges
- [ ] *D: Normal-mode particles* - Particle excitations as oscillations of natural parameters
- [ ] *E: Spatial embedding* - Auto-bin basis strings to L×L grid for field θ(x) visualization
- [ ] *F: Spectrum flow plots* - Time × log λ_k heatmaps showing quasi-symmetry plateaus
...
6. *Gradual Dephasing*: Cumulative time evolution (γ_τ = γ_per_block * τ) captures proper dephasing dynamics
...
```python
# Theory equation (1): ρ̄(τ) = (1-e^(-γτ))[diag(|a|²)] + e^(-γτ)|Ψ₀⟩⟨Ψ₀|
...
```python
# Create product of Bell pairs |Φ⁺⟩ = (|00⟩ + |11⟩)/√2
...
```python
# Steepest-Entropy-Ascent: ∂τρ = i[log ρ*, ρ]
# where ρ* = argmax S(ρ̃) subject to fixed marginals ρᵢ⁽¹⁾
def sea_evolution_step(self, rho, dt=0.01):
    # 1. Find ρ* that maximizes S(ρ̃) with fixed marginals
    # 2. Compute commutator [log ρ*, ρ]
    # 3. Evolve: ρ(τ + dτ) = ρ(τ) + i dτ [log ρ*, ρ]
...
def track_particle_decay(self, initial_rho, eigen_vector, n_steps=50):
    """Track standing-wave decay e^(-λt) of particle mode."""
...
3. *Particle Interpretation*: Normal modes as oscillations of natural parameters with effective masses λ_k
4. *Spatial Field Theory*: Natural parameters θ(x) as observable fields with Gauss law divergence
...
- *Backlog*: `2025-07-25_spectrum-flow-analysis.md` - Time × log λ_k heatmaps for quasi-symmetry plateaus