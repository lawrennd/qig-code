...
### Phase 1: Core σ Infrastructure
- [ ] Implement `validate_sigma(sigma, D)` function
- [ ] Implement `detect_sigma_structure(sigma, n_pairs, d)` function
- [ ] Implement `regularise_pure_state(psi, epsilon, sigma)` function

### Phase 2: Efficient Computation
- [ ] Extend `product_of_bell_states` with `bell_indices`
- [ ] Implement analytic θ for isotropic σ = I/D
- [ ] Implement analytic θ for pure-state σ = |ψ⟩⟨ψ|
- [ ] Implement `get_product_bell_parameters` with sigma parameter
- [ ] Implement `is_product_state` detection
- [ ] Implement `fisher_information_product` with block structure
- [ ] Add efficiency warnings when σ breaks assumptions
- [ ] Optimise `marginal_entropy_constraint` for product states

### Phase 3: Testing
- [ ] Test `validate_sigma` with valid/invalid inputs
- [ ] Test `detect_sigma_structure` for all σ types
- [ ] Verify analytic θ matches numeric logm (all σ types)
- [ ] Verify block-diagonal G matches full computation
- [ ] Test scaling: n=1,2,3,4 pairs with isotropic σ
- [ ] Test anisotropic σ gives different θ directions

### Phase 4: Documentation
- [ ] Document σ validation requirements
- [ ] Document efficiency implications of σ choice
- [ ] Add "north pole analogy" explanation
- [ ] Reference entropy_time_paths.ipynb analysis
