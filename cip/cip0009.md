...
**Root cause**: The current `qig/duhamel.py` uses **numerical integration** (trapezoid rule, 50 points) rather than analytical BCH formulas. The paper claims Lie closure allows avoiding explicit integration, but this is not implemented.

**Action**: Created backlog task `2025-12-08_bch-duhamel-implementation.md` to investigate and resolve. Possibilities:
1. Implement BCH-based analytical Duhamel for Lie-closed bases
2. Determine if paper's claim has unstated conditions  
3. Document the correct relationship if different from paper

### Current Test Strategy

Given the Duhamel limitation, tests verify what we **can** verify:
...
- ❌ **Blocked**: Full dynamical equivalence `∑_a (A@θ)_a ∂_a ρ = -i[H_eff, ρ]` (needs BCH-Duhamel)

The implementation is **functionally complete** for the extraction mechanics. The BCH identity verification awaits the Duhamel implementation improvement.
