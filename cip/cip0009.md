...
1. **Extraction formula**: `effective_hamiltonian_coefficients(A, theta, f_abc)` solves `F @ η = A @ θ` (where `F[r,c] = Σ_b f[r,b,c] θ_b`) to extract Hamiltonian coefficients; see *Correction note* below
...
### Correction note: extraction formula accuracy (added 2026-06-13)

The initial implementation recorded the extraction formula as `A @ θ = f @ η`
(short-hand for `einsum('abc,c->a', f_abc, eta)`).  This formulation was
**incorrect**: the right-hand side omits the contraction with θ.  The correct
formula is the adjoint-action form

```
(A @ θ)_r  ≈  Σ_{b,c} f[r,b,c] θ_b η_c   ≡  (F @ η)_r
```

i.e. `np.einsum('abc,b,c->a', f_abc, theta, eta)` in code.

Furthermore, the matrix `F[r,c] = Σ_b f[r,b,c] θ_b` is rank-deficient for
small θ (rank 12 of 15 for n_pairs=1, d=2), so the system `F @ η = A @ θ` is
**inconsistent**: the vector `A @ θ` has a component outside col(F) of
magnitude ~0.1·‖θ‖².  This is a second manifestation of the same BCH
approximation: the Kubo-Mori kernel embedded in A cannot be exactly captured
by the adjoint action of a Lie-algebra element.

**Consequence for tests**: `test_extraction_consistency_multiple_points` was
always failing with the wrong formula and a tolerance of 1e-6 that no solver
can achieve.  Fixed (2026-06-13) to use the correct einsum and a principled
O(‖θ‖²) bound (`0.5·‖θ‖²`).

### Final Test Strategy

With spectral Duhamel implemented and tested, we verify:
- ✅ H_eff is Hermitian (exact)
- ✅ H_eff is traceless (exact)
- ✅ Extraction formula `F @ η ≈ A @ θ` holds with BCH residual bounded by 0.5·‖θ‖²
...
- ✅ **Strong BCH identity documented**: `∑_a η_a ∂_a ρ = -i[H_eff, ρ]` does NOT hold in general
- ✅ **Rank-deficiency and extraction residual documented**: formula is O(‖θ‖²) approximate

### Completion Status

CIP-0009 is **functionally complete** with the following achievements:

1. ✅ **Core extraction implemented**: `F @ η = A @ θ` (F built from adjoint action of θ)
...
4. ✅ **Theoretical limitation documented**: Strong BCH identity proven false empirically (~14x error)
5. ✅ **Extraction residual documented**: Formula is O(‖θ‖²) approximate; floor ~0.1·‖θ‖²
6. ✅ **Tests corrected and guard against regression**: Wrong einsum fixed; BCH bound tested

**Completed enhancements**:
- [x] **Step 3**: End-to-end example with symbolic derivation (`examples/effective_hamiltonian_derivation.md`)
  - Complete mathematical derivation with code demonstrations
  - Executable test script (`test_effective_hamiltonian_derivation.py`) verifies all code
  - Symbolic forms: $H_{\text{eff}}(\theta) = \sum_c \eta_c(\theta) F_c$ with extraction identity
- [ ] **Step 5**: Theory documentation update (comprehensive documentation exists in backlog and example)
